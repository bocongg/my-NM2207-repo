...
**Solution:** CSV stands for "comma-separated values", and it is popular due to its compactness and plain text nature, allowing faster transfer and less corruption respectively (LinkedIn, 2023).
<br>
<br>
...
```
<br>
```{r, out.height= "100%",out.width= "100%",echo=TRUE,eval=TRUE,fig.cap="Screenshot of Help information on read_csv function."}
knitr::include_graphics("Screenshot of read_csv.png")
```
<br>

**Question 1.6:** What does the `skip` argument in the read_csv() function do?

**Solution:** It is to skip a number of lines before reading data.
<br>
<br>
...
ggplot(data=playlist_data, mapping=aes(x=Age,y=Rating)) + labs(x="Age", y="Rating.", title="Age of DJs vs Rating", caption="There appears to be no correlation between the age of DJs and their ratings.") + geom_point(shape="diamond")
